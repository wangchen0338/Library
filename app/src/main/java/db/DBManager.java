...
/**
 * 作者：wangchen on 2017/11/15 09:26
...
public class DBManager
{
    ...
    private DBManager(Context context, String dbName, boolean isDebug)
    {
        if (liteOrm == null)
        {
    ...
    public static DBManager getInstance(Context context, String dbName,boolean isDebug)
    {
        ...
        if (manager == null)
        {
            synchronized (DBManager.class)
            {
                if (manager == null)
                {
                    manager = new DBManager(context, dbName,isDebug);
                }
            }
        }
        return manager;
